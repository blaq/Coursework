...
celery = Celery('models',backend='redis://localhost',broker='redis://localhost')
...
class ActivityLog(db.Model):
    ...
    @classmethod
    def log_event(self, user, details):
        new_activity = {
            "user_id": user.id,
...
def post_activity(activity):
    ...
    url = log_url + "/api/activities/"
    try:
        r = requests.post(url, json=activity)
        if r.status_code == 201 or r.status_code == 200:
            logging.info(f"Post new activity SUCCESS at {url}")
        else:
